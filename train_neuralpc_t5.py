...
from dataset import NeuralPC_SPECIAL_TOKENS
...
def main():
    ...
    context_column = dataset_columns[0]
    response_column = dataset_columns[1]
    ...
    def preprocess_function(examples):
        inputs = examples[context_column]
        targets = examples[response_column]
        ...
        if trainer.is_world_process_zero():
            if training_args.predict_with_generate:
                ...
                if 'checkpoint' in model_args.model_name_or_path:
                    ckpt = (model_args.model_name_or_path).split("-")[-1]
                else:
                    ckpt = "000"
                output_test_preds_file = os.path.join(training_args.output_dir, f"ckpt{ckpt}-generations.txt")
